...
define('INSTAGRAM_TAG_ENDPOINTS_API_COUNT'			, 10);
define('INSTAGRAM_TAG_ENDPOINTS_API_MAX_PAGENATION'	, 3);
 
class ArchiveInstagramTagEndpoints {
	var $menuName;
	var $pageTitle;
	var $pageName;
	var $version;
...
<?php
			...
			$tag = isset($opt['tag']) ? $opt['tag']: null;
?>
			<table>
				<tr>
					<th>API client_id&nbsp;:</th>
				...
				<tr>
					<th>tag&nbsp;:</th>
					<td><input type="text" class="regular-text" name="options[tag]" value="<?=$tag?>" placeholder="tag"></td>
				</tr>
				<tr>
...
<?php
	...
	private function get_instagram($param, $url = null, $n = 0){
		...
		$data = http_build_query(array_intersect_key(
			array_merge($param, array('count' => INSTAGRAM_TAG_ENDPOINTS_API_COUNT)),
			array_flip(array('count', 'client_id', 'min_tag_id'))
		), '', '&');
...
$archiveInstagramTagEndpoints = new ArchiveInstagramTagEndpoints;
